...
def parse_args():
    ...
    parser.add_argument('--datasets', type=str, default='Human',
...
#                    with_prev_mask=True)
            model = HRNetModel(width=18, ocr_width=64, small=False, with_aux_output=True, use_leaky_relu=True,
                       use_rgb_conv=False, use_disks=True, norm_radius=5,
                       with_prev_mask=True)
            #para_state_dict = paddle.load('/mnt/haoyuying/iis/ritm_paddle_mac/experiments/noniterative_baselines/hrnet18/000/checkpoints/018.pdparams')
            para_state_dict = paddle.load('/mnt/haoyuying/iis/ritm_paddle_mac/experiments/iter_mask/cocolvis_hrnet18_ocr64_human/002/checkpoints/017.pdparams')
