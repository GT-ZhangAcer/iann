...
class ISTrainer(object):
    def __init__(self, model, cfg, model_cfg, loss_cfg,
        ...
        train_sample = paddle.io.DistributedBatchSampler(self.trainset,
                                                         batch_size=cfg.batch_size,
                                                         shuffle=True,
                                                         drop_last=True)
        self.train_data = paddle.io.DataLoader(self.trainset, batch_sampler=train_sample,num_workers=4)

        self.valset = valset
        val_sample = paddle.io.DistributedBatchSampler(self.valset,
                                                       batch_size=cfg.batch_size,
                                                       shuffle=False,
                                                       drop_last=True)
        self.val_data = paddle.io.DataLoader(self.valset, batch_sampler=val_sample, num_workers=4)
        ...
        self.lr_scheduler1 = paddle.optimizer.lr.MultiStepDecay(
            ...
            milestones=[20, 25], gamma=0.1)
        self.lr_scheduler2 = paddle.optimizer.lr.MultiStepDecay(
            learning_rate=optimizer_params['learning_rate'] * 0.1,
            milestones=[20, 25], gamma=0.1)
        
        optimizer1 = paddle.optimizer.Adam(learning_rate=self.lr, parameters=other_params)
        optimizer2 = paddle.optimizer.Adam(learning_rate=self.lr * 0.1, parameters=backbone_params)
...
#                 if global_step % log_iters == 0 and self.is_master:
...
#                         metric.log_states(self.sw, f'{log_prefix}Metrics/{metric.name}', val_global_step)
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with paddle.no_grad():
            #num_iters = random.randint(1, self.max_num_next_clicks-1)
            for click_indx in range(1):
            ...
            if self.with_prev_mask and self.prev_mask_drop_prob > 0 and last_click_indx is not None:
                ...
                prev_output[zero_mask] = paddle.zeros_like(prev_output[zero_mask])
            if not validation:
                self.net.train()
        batch_data[1] = points
